...
#i=0
#id=`sbatch --export=NONE --partition=shas --qos=normal --account=ucb62_summit3 --job-name=prep_stage --output=prep_output_%j.out --nodes=1 --ntasks=12 --time=24:00:00 prep_stack_par.sh | awk '{print $4}'`
...
# now submit the run_01 with the added dependency of the prep stage needing to run first
 ...
 #NUM_TASKS=12
 #id2=$id
 #id=`sbatch --depend=afterok:$id2 --export=NONE --partition=shas --qos=normal --account=ucb62_summit3 --job-name=${EXE_SCRIPT} --output=${EXE_SCRIPt}_output_%j.out --nodes=1 --ntasks=12 --time=24:00:00 ./runfiles_jobs.sh | awk '{print $4}'`
 ...
 id=`sbatch --export=NONE --partition=shas --qos=normal --account=ucb62_summit3 --job-name=${EXE_SCRIPT} --output=${EXE_SCRIPT}_output_%j.out --nodes=1 --ntasks=${NUM_TASKS} --time=24:00:00 ./runfiles_jobs.sh | awk '{print $4}'`
 ...
 NUM_TASKS=12
 id2=$id
 id=`sbatch --depend=afterok:$id2 --export=NONE --partition=shas --qos=normal --account=ucb62_summit3 --job-name=${EXE_SCRIPT} --output=${EXE_SCRIPT}_output_%j.out --nodes=1 --ntasks=${NUM_TASKS} --cpus-per-task=2 --time=24:00:00 ./runfiles_jobs.sh | awk '{print $4}'`
